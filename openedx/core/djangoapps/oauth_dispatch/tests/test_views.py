...
class TestAccessTokenView(AccessTokenLoginMixin, mixins.AccessTokenMixin, _DispatchingViewTestCase):
    ...
    def test_jwt_access_token(self, client_attr, mock_set_custom_metric):
        client = getattr(self, client_attr)
    ...
    @ddt.data(
        ('jwt', 'jwt'),
        (None, views.AccessTokenView.NO_TOKEN_TYPE_SUPPLIED_VALUE),
    )
    @ddt.unpack
    @patch('openedx.core.djangoapps.monitoring_utils.set_custom_metric')
    ...
    def test_access_token_metrics_for_bad_request(self, mock_set_custom_metric):
        ...
        expected_calls = [
            call('oauth_token_type', views.AccessTokenView.NO_TOKEN_TYPE_SUPPLIED_VALUE),
            call('oauth_grant_type', 'password'),
        ]
        mock_set_custom_metric.assert_has_calls(expected_calls, any_order=True)
